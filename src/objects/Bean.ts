...
export default class Bean extends Phaser.GameObjects.Container {
  ...
  private readonly BURST_SPEED = 200; // Decreased speed
  ...
  private drawJelly(tailOffset: Phaser.Math.Vector2) {
    ...
    const tailRadius = this.baseRadius * (1 - stretchFactor * 0.4);
    ...
    // Avoid drawing weird shapes if dist is too small or radii are too close
    // If dist is small, we just draw the head circle mostly.
    // We check if we can validly calculate the tangent.
    const radiusDiff = Math.abs(headRadius - tailRadius);

    if (dist <= radiusDiff + 0.1) {
        // Almost overlapping or contained. Just draw the larger circle (Head).
        // This avoids the 'snap' by falling back to the natural degenerate state.
        this.bodyGraphics.fillCircle(hx, hy, headRadius);
        this.bodyGraphics.strokeCircle(hx, hy, headRadius);
    } else {
        // Safe to calculate tangents
        let offsetAngle = Math.PI / 2;
        // Clamp input to acos to [-1, 1] just in case
        const ratio = (headRadius - tailRadius) / dist;
        const clampedRatio = Math.max(-1, Math.min(1, ratio));
        offsetAngle = Math.acos(clampedRatio);

        // Get angle from Head to Tail (Wait, earlier I did Tail to Head?)
        // Previous code: angle = Phaser.Math.Angle.Between(tx, ty, hx, hy); -> Tail to Head
        // And hx, hy is target.
        ...
        // Note: The previous code used +/- offsetAngle.
        // Here I'm being explicit.
        // We need to verify the points connectivity.
        // Head arc: from h1 to h2?
        // Tail arc: from t2 to t1?
        // Let's trace counter-clockwise.

        // Angle from T to H is `angle`.
        // Tangent points on Head are `angle - offsetAngle` (Right side if looking from T) and `angle + offsetAngle`.
        // Tangent points on Tail are `angle - offsetAngle` and `angle + offsetAngle`.

        // Path:
        // 1. Arc on Head from (angle + offsetAngle) to (angle - offsetAngle). (Long way around)
        // 2. Line to Tail (angle - offsetAngle).
        // 3. Arc on Tail from (angle - offsetAngle) to (angle + offsetAngle). (Short way, around the back)
        // 4. Line to Head (angle + offsetAngle).

        this.bodyGraphics.beginPath();
        // Arc on Head.
        // We want the "front" of the head. So we include the side away from the tail.
        // That corresponds to the range NOT covered by the tunnel.
        // So from +offset to -offset (CCW).
        this.bodyGraphics.arc(hx, hy, headRadius, angle + offsetAngle, angle - offsetAngle, false);

        // Line to Tail point corresponding to -offset
        this.bodyGraphics.lineTo(t1x, t1y);

        // Arc on Tail.
        // We want the back of the tail.
        // From -offset to +offset (CCW).
        this.bodyGraphics.arc(tx, ty, tailRadius, angle - offsetAngle, angle + offsetAngle, false);

        // Line back to Head start
    ...
    // Draw Orientation Dot (Simple Identity)
    // Always draw relative to Head (0,0) at angle 0 (Right, which is where the container points)
    const dotX = headRadius * 0.6;
    const dotY = 0;
    const dotSize = 3;

    this.bodyGraphics.fillStyle(0x000000, 0.8);
    this.bodyGraphics.fillCircle(dotX, dotY, dotSize);

    // Highlight (Shiny jelly) - kept
